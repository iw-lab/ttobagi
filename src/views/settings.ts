  ...
  void (async () => {
    ...
    fill(
      ...
      status !== 'ready'
        ...
        : null,
      // 🔴 여기에 영어가 없으면 「영어는 어디서 켜지?」 하고 설정을 뒤지게 된다(2026-09-10 사용자).
      //    영어 소리는 기기 목소리가 아니라 미리 구워 둔 원어민 음원이라 «고를 것이 없다» —
      //    없는 것이 아니라 고를 필요가 없다는 말을 여기서 해 줘야 한다.
      h('p', { class: 'muted small' }, '영어는 이 목록과 상관없어요. 원어민이 읽어 준 소리가 앱에 들어 있어서 기기 목소리가 없어도 그대로 들립니다. 영어 문항은 「급수표」 화면 맨 위 「국어 / 영어」에서 고르세요.'),
    );
  })();
